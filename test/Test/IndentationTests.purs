...
parseTest :: forall s a. Show a => Eq a => ParseErrorHuman__OnlyString s => s -> a -> IndentParser Identity s a -> Effect Unit
parseTest = mkParseTest (\input -> unwrap <<< runIndent <<< runParserT input)

parseErrorTestPosition :: forall s a. Show a => IndentParser Identity s a -> s -> Position -> Effect Unit
parseErrorTestPosition = mkParseErrorTestPosition (\input -> unwrap <<< runIndent <<< runParserT input)

parseErrorTestMessage :: forall s a. Show a => IndentParser Identity s a -> s -> String -> Effect Unit
parseErrorTestMessage = mkParseErrorTestMessage (\input -> unwrap <<< runIndent <<< runParserT input)

parseErrorTestPositionAndMessage :: forall s a. Show a => IndentParser Identity s a -> s -> String -> Position -> Effect Unit
parseErrorTestPositionAndMessage = mkParseErrorTestPositionAndMessage (\input -> unwrap <<< runIndent <<< runParserT input)
...
-- | ...
-- | ```
pTerm :: IndentParser Identity String String
pTerm = ((fromCharArray <$> NonEmpty.toArray) <$> Combinators.Array.many1 alphaNum) <* skipSpaces

pTaxonomy :: IndentParser Identity String Taxonomy
